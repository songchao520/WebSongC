...
layui.config({
    ...
    form.on('select(selectMsg)',function(data){
        if(data.value=="1"){
        	//加载数据数量
		    $.post(huobang_url+"/getOrderProblemCount",{loginId:huobang_loginId,isreading:0},function(data){
		    	msg = "allcxtj"
				if(data != "wdl"){
        	...
        	msg = "all"
        	 $.post(huobang_url+"/getOrderProblemCount",{loginId:huobang_loginId},function(data){
				if(data != "wdl"){
					currpage =1;
					maxlength = data;
					orderProblems();
    ...
    function getOrderData(){
		if(msg == "all"){
			$.post(huobang_url+"/getOrderProblems",{loginId:huobang_loginId,pagesize:pagesize,currpage:currpage,},function(data){
				if(data != "wdl"){
			...
			$.post(huobang_url+"/getOrderProblems",{loginId:huobang_loginId,pagesize:pagesize,currpage:currpage,isreading:0},function(data){
				if(data != "wdl"){
					orderProblemsData = data;
					$(".msgHtml").html(renderDate(data,currpage));
				}else{
    ...
    $("body").on("click",".reply_msg,.msgHtml .user_info h2,.msgHtml .msg_info>img",function(){
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            content : "orderProblemReply.html",
    ...
    $(".send_msg").click(function(){
        ...
        if(proval != ''){
        	...
        	$.post(huobang_url+"/updateOrderProblems",{loginId:huobang_loginId,recid:recid,replytime:adddate,replycontent:proval,reader:loginuser["recid"]},function(data){
				if(data != "wdl"){
					var replyHtml="";
