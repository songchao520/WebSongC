...
layui.config({
	...
	var form = layui.form(),
		...
		if(recid.toString() != "false"){
			$.post(huobang_url+"/getTruckType",{loginId:huobang_loginId,recid:recid},function(data){
				if(data != "wdl"){
					$("#imgsrcshow").attr("src",huobang_url+data[0]["imgsrc"]);
					delete data[0]["imgsrc"];
					for(var i in data[0]){
						$("#"+i).val(data[0][i]);
 	...
 	form.on("submit(addtrucktype)",function(data){
		...
		var kilometerprice = $("#kilometerprice").val();
		var startingprice = $("#startingprice").val();
		var createtime = formatTime(new Date());
		if(recid.toString() == "false"){
 			$.post(huobang_url+"/SaveTruckType",{loginId:huobang_loginId,tname:tname,chang:chang,wide:wide,high:high,loadweight:loadweight,price:price,kilometer:kilometer,createtime:createtime,kilometerprice:kilometerprice,startingprice:startingprice,loadvolume:0},function(data){
				if(data != "wdl"){
					compressImages(data);
 			...
 			$.post(huobang_url+"/updateTruckType",{loginId:huobang_loginId,tname:tname,chang:chang,wide:wide,high:high,loadweight:loadweight,price:price,kilometer:kilometer,createtime:createtime,kilometerprice:kilometerprice,startingprice:startingprice,loadvolume:0,recid:recid},function(data){
				if(data != "wdl"){
