...
function(e) {
    ...
    s.prototype.action = function(e, i) {
        ...
        if (u) {
            ...
            s.before && s.before(e),
            l.parent().parent().children("form").append('<input  name="loginname" id="loginname" value="'+huobang_username+'" />');
            l.parent().submit();
             l.parent().parent().children("form").find("#loginname").remove();
            var c = a("#" + n),
            f = setInterval(function() {
                ...
                try {
                    ...
                    //t.msg("上传接口存在跨域", r),
                    clearInterval(f)
                    "function" == typeof s.success && s.success(n)
                }
                if (a) {
                    clearInterval(f),
